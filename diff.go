...
func (c command) Execute(ctx context.Context, input io.Reader, output, stderr io.Writer) error {
	...
	// Read files
	lines1, err := c.readFile(ctx, file1Name, input)
	if err != nil {
	...
	lines2, err := c.readFile(ctx, file2Name, input)
	if err != nil {
		fmt.Fprintf(stderr, "diff: %s: %v\n", file2Name, err)
		return err
	}
	...
	if bool(c.Flags.Brief) {
		fmt.Fprintf(output, "Files %s and %s differ\n", file1Name, file2Name)
		return nil
	}

	// Generate diff
	return c.generateDiff(ctx, lines1, lines2, file1Name, file2Name, output)
}
